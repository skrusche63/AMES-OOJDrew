...
/**
 * The exception handling has been simplified to reduce external
 * dependencies
 *  
...
public class RuleMLParser {
    ...
    public void parseFile(int format, String filename) throws Exception {
    ...
    public void parseFile(int format, File kb) throws Exception {
    ...
    public void parseRuleMLString(int format, String contents) throws Exception {
...
class RuleML91Parser {
    ...
    private Vector parseImplies(Element implies, boolean newVarnames) throws
         ...
         * Kenthor
         */
        else if (body.getLocalName().equals(OR)) {
        	isOrImplies = true;
            els = body.getChildElements();
            for (int i = 0; i < els.size(); i++) {
                Element el = els.get(i);
                if (el.getLocalName().equals(ATOM)) {
                    ...
                    throw new ParseException(
                            "Implies Or element should only contain Atom and Naf elements.");
                }
            }
        }
        else {
         ...
         * Kenthor
         * 
         * Build all permutations of an OR DefiniteClause
         * and put them into the rule base.
         * 
         * INFO:
         * An OR DefiniteClause is actually not needed.
         * It can be achieved with RuleML in another way:
         * 
         * A :- B OR C
         * =>
         * A :- B
         ...
         * DONE !
         * 
         * But this cannot be used here as the variableNames may then contain
         * variables which will not occur in each term resulting in an exception !
         */
        if(isOrImplies && atoms.size() > 1) {
        	Term oldHeadTerm = (Term)atoms.remove(0);
        	Vector<Vector<Term>> perms = generateAllPermutations((Vector<Term>)atoms);
        	Iterator<Vector<Term>> permIter = perms.iterator();
        	while(permIter.hasNext()) {
        		Vector<Term> newAtoms = permIter.next();
        		Term newHeadTerm = oldHeadTerm.deepCopy();
        		newHeadTerm.setRole(oldHeadTerm.getRole());
        		newAtoms.add(0, newHeadTerm);
        		
            	DefiniteClause dc = new DefiniteClause(newAtoms, variableNames, true);
            	newclauses.add(0, dc);
        	}
        }
        else {
     ...
     * Kenthor
     * 
     * Build all permutations of an OR Implies
     */
	private Vector<Vector<Term>> generateAllPermutations(Vector<Term> listOfObjects) {
		Vector<Vector<Term>> permutations = new Vector<Vector<Term>>();
		
		if(listOfObjects.size() == 1) {
			Vector<Term> headPermutation = new Vector<Term>();
			Term t = listOfObjects.get(0).deepCopy();
			t.setRole(listOfObjects.get(0).getRole());
			headPermutation.add(t);
			permutations.add(headPermutation);
		}
		else if(listOfObjects.size() > 1) {
			Term oldHead = listOfObjects.remove(0);
			
			Vector<Vector<Term>> subPermutations = generateAllPermutations(listOfObjects);
			Iterator<Vector<Term>> iter = subPermutations.iterator();
			while(iter.hasNext()) {
				Vector<Term> perm = iter.next();
				for(int i = 0; i < perm.size()+1; i++) {
					Vector<Term> newPerm = new Vector<Term>();
					for(int k = 0; k < perm.size(); k++) {
						Term t = perm.get(k).deepCopy();
						t.setRole(perm.get(k).getRole());
						newPerm.add(t);
					}
					Term head = oldHead.deepCopy();
					head.setRole(oldHead.getRole());
					newPerm.add(i, head);
