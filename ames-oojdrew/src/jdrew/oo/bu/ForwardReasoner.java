...
public class ForwardReasoner {
	...
	// Kenthor disabled as it seems unused outside the GUI, which are not used in this project
	//public static int rem = jdrew.oo.gui.BottomUpGUI.currentParser;

	// Kenthor
    private Vector undecidedNAFs;
    private Vector unsuccessfulNAFs;
    private Hashtable<Integer,Vector<DefiniteClause>> undecidedNAFRules;
    private Vector<Term> nafHeads;
    ...
    public ForwardReasoner() {
        ...
        // Kenthor
        undecidedNAFs = new Vector(); 
        unsuccessfulNAFs = new Vector();
        undecidedNAFRules = new Hashtable<Integer,Vector<DefiniteClause>>();
        nafHeads = new Vector<Term>();
    ...
    public void runForwardReasoner() {
        ...
        boolean beforeFirstNAFIteration = true;
        while(newFacts.size() > 0 || beforeFirstNAFIteration || !undecidedNAFRules.isEmpty()) {
	        while (newFacts.size() > 0) {
	            ...
	            //System.out.println("Add Old Fact " + sym + ": " + dc.toPOSLString());
	            ...
	            if (rules.containsKey(sym)) {
	                ...
	                while (it.hasNext()) {
	                    ...
	                    //System.out.println("RUN - Unifying rule " + rule.toPOSLString() + " with fact " + dc.toPOSLString());
	                                       
	                    Unifier u = new Unifier(dc, rule);
	                    
	                    if (u.unifies()) {
	                        // new fact unifies with a rule - process the resolvent
	                        DefiniteClause r = null;
							try {
								r = u.resolvent();
							} catch (Exception e) {
								e.printStackTrace();
								//System.out.println("DC: " + dc.toPOSLString() + " RULE: " + rule.toPOSLString());
							}
	                        //logger.debug("Unified - resolvent: " + r.toPOSLString());
	                        //System.out.println("RUN - Unified - resolvent: " + r.toPOSLString());
	                        al.add(r);
	                    }
	                }
	        ...
	        // Kenthor
	        beforeFirstNAFIteration = false;
	        
	        Enumeration<Term> nafTerms = nafHeads.elements();
	        // Go through the NAF heads list
	        // and resolve the first NAF rule set found
	        while(nafTerms.hasMoreElements()) {
	        	Term t = nafTerms.nextElement();
	        	
	        	Integer nafSymbol = t.getSymbol();
	        	if(undecidedNAFRules.containsKey(nafSymbol)) {
	    	        boolean newResolvents = false;

	    	        Vector<DefiniteClause> v = undecidedNAFRules.remove(nafSymbol);
	        		Iterator<DefiniteClause> nafRulesIter = v.iterator();
	        		while(nafRulesIter.hasNext()) {
	        			DefiniteClause dc = nafRulesIter.next();
	        			if(resolveNAFRule(dc)) {
	        				newResolvents = true;
	        			}
	        		}
	        		
	    	        // If new results (reduced rules or new facts) have been resolved
	    	        // stop processing of further NAF rules sets and process the new results
	        		if(newResolvents)
	        			break;
	        	}
	        }
     ...
     * Kenthor
     * 
     * Added to resolve a NAF rule and either ignore it (NAF terms are all true)
     * or remove the NAF element (at least one NAF term is false) and bring the
     * resolved rule back into the bottom up process.
     */
    private boolean resolveNAFRule(DefiniteClause dc) {
    	if(dc.atoms[1].getSymbol() != SymbolTable.INAF) {
    		return false;
    	}
    	
    	Term[] subTerms = dc.atoms[1].getSubTerms();
    	if(subTerms.length != 1) {
    		return false;
    	}
    	
    	Term dcNAFTerm = subTerms[0];
    	boolean nafIsTrue = false;
    	boolean nafUnifies = false;
    	
    	/* DEBUG only
    	if(dc.variableNames.length > 0)
    		System.out.println("Open Vars for " + dc.toPOSLString());
    	for(int i = 0; i < dc.variableNames.length; i++)
    		System.out.println("Vars: " + dc.variableNames[i]);

		for(int k = 0; k < dcNAFTerm.subTerms.length; k++)
			System.out.println("Subterm: " + dcNAFTerm.subTerms[k].toPOSLString(true));
		*/
    	
		Integer sym = new Integer(dcNAFTerm.getSymbol());
		if(oldFacts.containsKey(sym)) {
			Vector v = (Vector)oldFacts.get(sym);
			
			Iterator factsIter = v.iterator();
			while(factsIter.hasNext()) {
				DefiniteClause oldFact = (DefiniteClause)factsIter.next();
    			Unifier u = new Unifier();
    			
    			//
    			if(oldFact.atoms[0].compareTo(dcNAFTerm) == 0) {
    				// System.out.println("COMP-EQUAL: " + oldFact.atoms[0].toPOSLString(true) + " " + dcNAFTerm.toPOSLString(true));	    				
    				nafIsTrue = true;
    				break;
    			}
    			//
    			else if(u.unify(oldFact.atoms[0], dcNAFTerm)) {
    				// System.out.println("UNIFIES: " + oldFact.atoms[0].toPOSLString(true) + " " + dcNAFTerm.toPOSLString(true));
    				nafUnifies = true;
    				// Here it gets a little complicated and probably not very efficient
    				
    				// Does not make any sense - REWORK by check, if there are other non-NAF Terms in the DC
    				// Move the next non-NAF term to the top of the body list
    				
//	    	    		Vector<Term> extendedAtoms = new Vector<Term>();
//	    	    		Term head = dc.atoms[0].deepCopy();
//	    	    		head.setRole(dc.atoms[0].getRole());
//	    	    		extendedAtoms.add(head);
//	    	    		extendedAtoms.add(dcNAFTerms[i]);
//	    	    		for(int n = 1; n < dc.atoms.length; n++) {
//	    	    			// TODO: deref needed !?
//	    	    			// TODO: apply needed !?
//	    	        		Term body = dc.atoms[n].deepCopy();
//	    	        		body.setRole(dc.atoms[n].getRole());
//	    	        		extendedAtoms.add(body);
//	    	    		}
//	    	    		
//	    	    		Vector<String> variableNames = new Vector<String>();
//	    	    		for(int n = 0; n < dc.variableNames.length; n++) {
//	    	    			variableNames.add(dc.variableNames[n]);
//	    	    		}
//
//	    	    		DefiniteClause newDC = new DefiniteClause(extendedAtoms, variableNames);
//	    	    		System.out.println("New DC: " + newDC.toPOSLString());
//
//	    	    		Unifier test = new Unifier(oldFact, newDC);
//	    				DefiniteClause resolvent = null;
//	    				if(test.unifies()) {
//	    					//process(test.resolvent());
//	    				}
//	    	    		
//	    				nafResolvents = true;
    			}
    			else {
    				//System.out.println("NOT EQUAL: " + oldFact.atoms[0].toPOSLString(true) + " " + dcNAFTerm.toPOSLString(true));
    			}	    			
			}
		}    		
    	...
    	if(nafIsTrue) {
    		// The NAF atom is true therefore the rule will NOT be true
    		return false;
    	}
    	else if(nafUnifies) {
    		// The NAF atom unifies with a fact.
    		// Try to bring an non-NAF atom to the front of the body for further resolving.
    		Vector<Term> reorderedAtoms = new Vector<Term>();
    		
    		Term naf = dc.atoms[1].deepCopy();
    		naf.setRole(dc.atoms[1].getRole());
    		
    		int index = -1;
    		for(int i = 2; i < dc.atoms.length; i++) {
    			if(dc.atoms[i].getSymbol() != SymbolTable.INAF) {
    				index = i;
    				break;
    			}
    		}
    		...
    		if(index != -1) {
    			// Reorder the body terms of the rule and bring the new rule into the process again. 
    			//
    			// TODO: deref needed !?
    			// TODO: apply needed !?
    			Term head = dc.atoms[0].deepCopy();
    			head.setRole(dc.atoms[0].getRole());
    			reorderedAtoms.add(head);
    		
    			Term reorder = dc.atoms[index].deepCopy();
    			reorder.setRole(dc.atoms[index].getRole());
    			reorderedAtoms.add(reorder);
    		
    			for(int i = 1; i < dc.atoms.length; i++) {
    				if(i != index) {
    	        		Term body = dc.atoms[i].deepCopy();
    	        		body.setRole(dc.atoms[i].getRole());
    	        		reorderedAtoms.add(body);
    				}
    			}
	    		...
	    		DefiniteClause newDC = new DefiniteClause(reorderedAtoms, variableNames);
	
	    		process(newDC);
	        	return true;
    		}
    		else {
        		// The NAF atom unifies with a fact but there is no further non-NAF term to be resolved
    			// therefore the rule will NOT be true
    			return false;
    		}
    	}
    	else {
    		Vector<Term> reducedAtoms = new Vector<Term>();
    		Term head = dc.atoms[0].deepCopy();
    		head.setRole(dc.atoms[0].getRole());
    		reducedAtoms.add(head);
    		for(int i = 2; i < dc.atoms.length; i++) {
    			// TODO: deref needed !?
    			// TODO: apply needed !?
        		Term body = dc.atoms[i].deepCopy();
        		body.setRole(dc.atoms[i].getRole());
        		reducedAtoms.add(body);
    		}
    		...
    		DefiniteClause newDC = new DefiniteClause(reducedAtoms, variableNames);
        ...
        if (oldFacts.containsKey(sym)) {
            ...
            while (it.hasNext()) {
                ...
                if (s.subsumedBy(dc)) {
                    ...
                    //           dc.toPOSLString());
                    //System.out.println(fact.toPOSLString() + " is subsumed by " + dc.toPOSLString());
                    return true;
                }
    ...
    private void process(DefiniteClause dc) {
        ...
        if (dc.atoms.length == 1) {
            ...
            //System.out.println("Add New Fact: " + dc.toPOSLString());
        	
            newFacts.add(dc);
        } 
        else {
            //logger.debug("Processing " + dc.toPOSLString());
            //System.out.println("PROC - Processing: " + dc.toPOSLString());
            //System.out.println("PROC - Atoms1: " + dc.atoms[1].toPOSLString(true));
            ...
            while (ofsit.hasNext()) {
                ...
                //System.out.println("PROC - Unifying new rule " + dc.toPOSLString() + " with old fact " + oldfact.toPOSLString());
                if (u.unifies()) {
                    DefiniteClause nr = u.resolvent();
                    newResults.add(nr);
                    // logger.debug("Unified - resolvent: " + nr.toPOSLString());
                    //System.out.println("PROC - Unified - resolvent: " + nr.toPOSLString());
                }
            }
            ...
            //System.out.println("Add rule: " + dc.toPOSLString());
            /*
        	for(int i = 0; i < dc.variableNames.length; i++)
        		System.out.println("Vars: " + dc.variableNames[i]);
			*/
             ...
             * Kenthor
             * 
             * Remember all NAF rules for later resolution.
             */
            if(dc.atoms[1].getSymbol() == SymbolTable.INAF) {
            	Integer headSym = new Integer(dc.atoms[0].getSymbol());
                if(undecidedNAFRules.containsKey(headSym)) {
                    Vector<DefiniteClause> v = undecidedNAFRules.get(headSym);
                    v.add(dc);
                } else {
                    Vector<DefiniteClause> v = new Vector<DefiniteClause>();
                    v.add(dc);
                    undecidedNAFRules.put(headSym, v);
                }
            }
        }
    }
	...
	public void buildPrecedenceGraph(){
		 ...
		 * Kenthor
		 * 
		 * Vector of all remembered NAF nodes.
		 */
		Vector<Node> nafNodes = new Vector<Node>();
		...
		while(e.hasMoreElements()){	
			...
			while(it.hasNext()){
				 ...
				 * Kenthor
				 * 
				 * Remember all the nodes which represent rules that have NAF terms.
				 */
				if(hasNAF && !nafNodes.contains(sink)) {
					nafNodes.add(sink);
				}
			}//each term
		}//each rule		
		 ...
		 * Kenthor
		 * 
		 * Creating a topological sorted list of all NAF rule heads by
		 * analyzing the respective nodes from the directed graph.
		 * 
		 * This is needed to resolve the open NAF rules in the correct order
		 * during the bottom up reasoning.
		 */
		List sortedNAFNodes = null;
		//System.out.println("NAF Nodes " + nafNodes.size());
		try {
			sortedNAFNodes = dg.topologicalSort(nafNodes);
			Iterator nafIter = sortedNAFNodes.iterator();
			while(nafIter.hasNext()) {
				Node n = (Node)nafIter.next();
				//System.out.println("NODE List: " + ((Term)n.getWeight()).toPOSLString(true));
				nafHeads.add((Term)n.getWeight());
			}
		} catch (GraphActionException e1) {
			// Problem, but if this happens, the rules are not stratifiable at all and thus not
			// completely solvable !
