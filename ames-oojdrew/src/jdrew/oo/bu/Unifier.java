...
public class Unifier {
    ...
    public DefiniteClause resolvent() {
        ...
        Term head = apply(atoms[RULE][0]); // Kenthor
        newatoms.add(head);
        ...
        if(this.clauses[RULE].isOrImplies()) {
        	// TODO: Check, if all head variables are set ?
        	Vector headAtom = new Vector();
        	headAtom.add(head);
        	dc = new DefiniteClause(headAtom, variableNames);
        }
        else {
        	dc = new DefiniteClause(newatoms, variableNames);
        }
